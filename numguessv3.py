...
def check_to_change_previous_guesses(guess):
...
def run_guessing_game(guess_count, victory):
    while guess_count > 0 and victory == False:
        victory = check_if_guess_is_correct(
            int(input("What is your guess? ")), guess_count, victory)
...
def check_if_guess_is_correct(guess, guess_count, victory):
    check_to_change_previous_guesses(guess)
